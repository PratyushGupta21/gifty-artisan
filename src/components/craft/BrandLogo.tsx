...
export function BrandLogo({ className = "", hideTextOnMobile = false, textColor }: BrandLogoProps) {
  const textClr =
    ...
    (className.includes("text-")
      ? ""
      : "bg-gradient-to-r from-[#E0BC88] via-[#D4A373] to-[#B58A52] bg-clip-text text-transparent");
  ...
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      ...
      <div className="relative size-9 shrink-0" aria-hidden>
        ...
        <div className="absolute inset-x-0.5 bottom-0 h-[58%] rounded-[5px] bg-[#B85B3A] shadow-sm shadow-[#B85B3A]/40" />
        ...
        {/* ── Memory bloom petals (blush / gold) ── */}
        ...
        <div className="absolute right-[14%] top-[10%] h-[28%] w-[22%] rotate-[18deg] rounded-[50%] bg-[#D4A373] opacity-85" />

        {/* Tiny sparkle accent — a pinpoint of warmth */}
        <div className="absolute right-[8%] top-[4%] size-[12%] rounded-full bg-[#E0BC88] opacity-90" />

        {/* Inner box shadow line */}
        <div className="absolute inset-x-2 bottom-[14%] h-[2px] rounded-full bg-[#181310] opacity-20" />
      </div>

      {/* ─── Brand Typography ─── */}
      <span
        className={`font-[family-name:var(--font-display)] text-lg font-bold tracking-tight ${textClr} ${
